...
describe('zonedDateTime', () => {
  ...
  const zdtAction = temporalToMaxValue(req);
  ...
  test('should return value unchanged for value before requirement', () => {
    const value = Temporal.ZonedDateTime.from('2024-01-01T00:00:00+00:00[UTC]');
    expect(zdtAction['~run']({ typed: true, value }, {})).toStrictEqual({ typed: true, value });
  });

  test('should clamp value to requirement for value after requirement', () => {
...
describe('instant', () => {
  ...
  const instantAction = temporalToMaxValue(req);
...
describe('plainDateTime', () => {
  ...
  const dtAction = temporalToMaxValue(req);
...
describe('plainDate', () => {
  const requirement = Temporal.PlainDate.from('2024-06-01');
  const dateAction = temporalToMaxValue(requirement);
...
describe('plainTime', () => {
  ...
  const timeAction = temporalToMaxValue(req);
